...
public class Main {
	...
	// (1p) Choose the best key length for DSA, shortly explain your choice.
	// As I managed to google, support od DSA key length on many system is limited to 1024 bit
	// So, to be sure that data wil be decrypted on any partner service, I'll leave it 1024.
	// Actually when tried put 2048, got
	// "Modulus size must range from 512 to 1024 and be a multiple of 64"
	private static int dsaKeySize = 1024;
	// (2p) Choose the best key length for RSA, shortly explain your choice.
	// As there is no small limitations to RSA key length, I took recommended size of 2048.
	...
	private static void doEncryption(byte[] data) throws IOException {
		...
		// GS looks like a hint %)
		System.out.println("\n> Testing public key encryption...");

		KeyPair rsaKeyPair = MyKeyGenerator.generateRsaKeyPair(rsaKeySize);
		// Replace `null`-s with proper keys from rsaKeyPair
		// GS encrypt with public, decrypt with private
	...
	private static void doSigning(byte[] data) throws IOException {
		...
		// Replace `null`-s with proper keys from dsaKeyPair
		// GS sign with private, verify with public
